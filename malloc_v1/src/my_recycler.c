...
void recycler_create(struct recycler **recycler_ptr, size_t block_size, size_t total_size, void *start_addr) {
    ...
    if (block_size % ALIGNMENT != 0 || block_size == 0 || total_size == 0 || recycler_ptr == NULL || *recycler_ptr == NULL) {
    ...
    for (size_t i = 1; i < recycler->capacity; i++) {
        ...
        current_block->next = (struct free_list *)((char *)current_block + block_size);
...
void *recycler_allocate(struct recycler *recycler)
{
    ...
    recycler->free = allocated_block->next; // Move the free pointer to the next free block
